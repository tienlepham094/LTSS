...
void Dijkstra(int loc_mat[], int loc_dist[], int loc_pred[], int loc_n, int n,
              MPI_Comm comm, double *T_w_com, double *T_wo_com);
...
int main(int argc, char **argv)
{
    ...
    double start_time, end_time, T_w_com, T_wo_com;
    MPI_Comm comm;
    ...
    Dijkstra(loc_mat, loc_dist, loc_pred, loc_n, n, comm, &T_w_com, &T_wo_com);
 ...
 *             loc_pred: loc_pred[v] = predecessor of v on a shortest path from source to v
 *             T_w_com: Total time with communication
 *             T_wo_com: Total time without communication
 */
void Dijkstra(int loc_mat[], int loc_dist[], int loc_pred[], int loc_n, int n,
              MPI_Comm comm, double *T_w_com, double *T_wo_com)
{
    ...
    int my_min[2];
    int glbl_min[2];
    double start_time, end_time, comm_time = 0.0;
    ...
    for (i = 0; i < n - 1; i++)
    {
        ...
        /* Get the starting time for communication */
        start_time = MPI_Wtime();
        ...
        */
        ...
        /* Get the ending time for communication */
        end_time = MPI_Wtime();
    ...
    /* Set the total time with and without communication */
    *T_w_com = comm_time;
    *T_wo_com = (end_time - start_time) - comm_time;
}
